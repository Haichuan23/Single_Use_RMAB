...
def fluid_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    objective = cp.Maximize(cp.sum([mu[mu_index(n, s, 1, t)] * r[s]
                                    for n in range(N)
    ...
    print("Running Time: {:.5f} seconds".format(running_time))

    return optimal_reward, average_reward, fluid_std, running_time
...
def random_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    for sim in range(num_simulations):
        ...
        for t in range(T):
            ...
            for arm in activated_arms:
                ...
                random_total_reward += r[arm_states[type][mem]]
                current_state = arm_states[type][mem]
                if current_state >= S:  # Dummy states are indexed after normal states
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (Random policy): {average_random_total_reward}, stdev is {random_std}")
    print("running time: {:.5f} seconds".format(running_time))
...
def no_pull_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (No-Pull policy): {average_no_pull_total_reward}, stdev is {no_pull_std}")
    print("running time: {:.5f} seconds".format(running_time))
    return average_no_pull_total_reward, no_pull_std, running_time
...
def Q_difference_index(rewards, transition_probabilities, states, T):
    ...
    for s in states:
        Q_difference_indices[T][s] = rewards[s]
...
def calculate_all_arm_Q_difference(N, rewards, transition_probabilities, S, T):
    ...
    for arm in range(N):
        Q_difference_all_arm[arm] = dict_to_matrix(
            Q_difference_index(rewards, transition_probabilities[arm], states, T))
...
def Q_difference_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    for sim in range(num_simulations):
        ...
        for t in range(T):
            ...
            for arm in activated_arms:
                type, mem, current_state, _ = arm
                Q_difference_reward += r[arm_states[type][mem]]
                # if current_state >= S:  # Dummy states are indexed after normal states
                next_state = np.random.choice(range(S_prime), p=P[type, current_state, 1, :])
                arm_states[type][mem] = next_state
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (General Q difference index policy): {average_general_Q_difference_total_rewards}, stdev is {Q_difference_std}")
    print("running time: {:.5f} seconds".format(running_time))
    return average_general_Q_difference_total_rewards, Q_difference_std, running_time
...
'''
Infinite Whittle Index Policy with dummy states
'''
def value_iteration(rewards, transition_probabilities, discount_factor, states, lambda_value, max_iterations=10000,
    ...
    for iteration in range(max_iterations):
        ...
        for s in states:
            ...
            Q_s_0_new[s] = 0 + lambda_value + discount_factor * future_reward_0
...
def calculate_whittle_index_with_dp_binary_search(rewards, transition_probabilities, discount_factor, S,
                                                  max_iterations=100,
                                                  delta=1e-6):
...
def calculate_all_arm_whittle(N, rewards, transition_probabilities, S, discount_factor=0.99):
    whittle_all_arm = np.zeros((N, S))

    for arm in range(N):
        whittle_all_arm[arm] = calculate_whittle_index_with_dp_binary_search(rewards, transition_probabilities[arm],
                                                                             discount_factor, S)
...
def infinite_whittle_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    for sim in range(num_simulations):
        ...
        for t in range(T):
            ...
            if len(current_indices) >= K:
                ...
                activated_arms = current_indices
            ...
            for arm in activated_arms:
                type, mem, current_state, _ = arm
                whittle_total_reward += r[arm_states[type][mem]]
                # if current_state >= S:  # Dummy states are indexed after normal states
                next_state = np.random.choice(range(S_prime), p=P[type, current_state, 1, :])
                arm_states[type][mem] = next_state
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (General Infinite Whittle index policy): {average_general_whittle_total_reward}, stdev is {general_whittle_std}")
    print("running time: {:.5f} seconds".format(running_time))
...
def original_whittle_policy(N, T, S, A, K, group_member_num, P_original, P, r, num_simulations):
    # Calculate Whittle index for normal states
    start_time = time.time()
    original_whittle_indices = calculate_all_arm_whittle(N, r, P_original, S)

    # Output the Whittle indices
    print("Original Whittle Indices (no dummy states):")
    ...
    for sim in range(num_simulations):
        ...
        for t in range(T):
            ...
            for type in range(N):
                for mem in range(group_member_num):
                    ...
                    if activation_mask[type][mem] == 0:
                        whittle_index_value = -10000  ## make sure that it will not be selected
                    else:
            ...
            for arm in activated_arms:
                type, mem, current_state, _ = arm
                original_whittle_total_reward += r[arm_states[type][mem]]
                next_state = np.random.choice(range(S), p=P_original[type, current_state, 1, :])
                arm_states[type][mem] = next_state
            ...
            for type in range(N):
                for mem in range(group_member_num):
                    if (type, mem) not in [(arm[0], arm[1]) for arm in activated_arms]:
                        current_state = arm_states[type][mem]
                        next_state = np.random.choice(range(S), p=P_original[type, current_state, 0, :])
                        arm_states[type][mem] = next_state
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (General Infinite Whittle index policy): {average_original_whittle_total_reward}, stdev is {original_whittle_std}")
    print("running time: {:.5f} seconds".format(running_time))
    ...
    for time in range(T - 1, t - 1, -1):
        for s in states:
            if (time == T - 1):
                ...
                future_reward_1 = sum(
                    transition_probabilities[s, 1, next_state] * max(Q_s_1[time + 1][next_state],
                                                                     Q_s_0[time + 1][next_state])
                    for next_state in states
                )

                future_reward_0 = sum(
                    transition_probabilities[s, 0, next_state] * max(Q_s_1[time + 1][next_state],
                                                                     Q_s_0[time + 1][next_state])
                    for next_state in states
                )

            Q_s_1[time][s] = rewards[s] + future_reward_1
            Q_s_0[time][s] = 0 + future_reward_0 + lambda_value
...
def calculate_all_arm_finite_whittle(N, rewards, transition_probabilities, S, T):
    ...
    for arm in range(N):
        finite_whittle_indices = calculate_finite_whittle_with_dp_binary_search(rewards, transition_probabilities[arm],
                                                                                S, T)
...
def finite_whittle_policy(N, T, S, S_prime, A, K, group_member_num, P, r, num_simulations):
    ...
    for sim in range(num_simulations):
        ...
        for t in range(T):
            ...
            for arm in activated_arms:
                type, mem, current_state, _ = arm
                finite_whittle_reward += r[arm_states[type][mem]]
                # if current_state >= S:  # Dummy states are indexed after normal states
                next_state = np.random.choice(range(S_prime), p=P[type, current_state, 1, :])
                arm_states[type][mem] = next_state
    ...
    print(
        f"Average total achieved value over {num_simulations} simulations (General Finite Whittle index policy): {average_general_finite_whittle_total_rewards}, stdev is {finite_whittle_std}")
    print("running time: {:.5f} seconds".format(running_time))
...
def plot_methods_with_confidence(data, N, T, K, group_mem, num_simulations, type, lower_bound_method, show_error=False):
    ...
    if (show_error):
        bars = plt.bar(methods, values, color=colors[:len(methods)], yerr=errors, capsize=5,
