...
def fluid_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
    ...
    for t in range(T):
        constraints.append(cp.sum([mu[mu_index(n, s, 1, t)]
                               for n in range(N)
                               for s in range(S)]) <= K/group_member_num)
    ...
    if problem.status in ["infeasible", "unbounded"]:
        ...
        if optimal_mu is not None:
            optimal_mu_4D = optimal_mu.reshape((N, S, A, T))
...
'''
def random_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
...
'''
def no_pull_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
...
'''
def Q_difference_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
    # Calculate Whittle index for normal states
    start_time=time.time()
    general_Q_difference_indices = calculate_all_arm_Q_difference(N, r, P, S, T)
...
def calculate_whittle_index_with_dp(rewards, transition_probabilities, discount_factor, S, max_iterations=100,
                                    delta=1e-6):
...
def calculate_whittle_index_with_dp_binary_search(rewards, transition_probabilities, discount_factor, S, max_iterations=100,
                                    delta=1e-6):
...
'''
def infinite_whittle_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
    # Calculate Whittle index for normal states
    start_time=time.time()
    general_whittle_indices = calculate_all_arm_whittle(N, r, P, S)
...
'''
def finite_whittle_policy(N, T, S, A, K, group_member_num, P, r, num_simulations):
    # Calculate Whittle index for normal states
    start_time=time.time()
    general_finite_whittle_indices = calculate_all_arm_finite_whittle(N,r,P,S, T)
...
def plot_methods_with_confidence(data, N, T, K, group_mem, num_simulations, type, lower_bound_method, show_error = False):
    ...
    plt.tight_layout()
    plt.savefig(f'N={N},T={T},B={K},group_size={group_mem},type={type}_noonepull.png')
...
def plot_running_time(running_time_dict, N, T, K, group_mem, type):
    ...
    plt.savefig(f'RT_N={N},T={T},B={K},group_size={group_mem},type={type}_noonepull.png')
